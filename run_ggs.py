...
ELITE_K       = 1                 # number of elites to keep each gen
CXPB          = 0.7               # crossover prob
MUTPB         = 0.7               # per-gene mutation prob (applied independently)
K_SIGMA       = 3.0
SEED          = 42                # set to None for non-deterministic runs

# Genome clamping (sizes are in pixels before logging)
MIN_SCALE_SPLATS = 5.0            # min radius in pixels
...
# ============================================================
# Genome helpers
# Each splat row: [x, y, a_log, b_log, theta, r, g, b, alpha]  -> total 9 entries
# ============================================================
...
def clamp_genome(ind:torch.Tensor) -> torch.Tensor:
    """
    Clamp/wrap genome (with theta at column 4).
    """
    # x,y
...
def fitness_mse(ind_axes_angle:torch.Tensor, target:torch.Tensor) -> float:
    # Convert to renderer format (with Cholesky params)
...
def mutate_individual(ind:torch.Tensor) -> torch.Tensor:
    with torch.no_grad():
        ...
        # alpha
        mask = torch.rand_like(ind[:,8]) < MUTPB   # <-- alpha is col 8
        ind[mask,8] += torch.randn((mask.sum(),), device=ind.device) * MUT_SIGMA["alpha"]
...
if __name__ == "__main__":
    ...
    if best_ind is not None:
        ...
        # convert to renderer format (Cholesky)
        ...
        Image.fromarray(img8).save("ga_splats.png")
